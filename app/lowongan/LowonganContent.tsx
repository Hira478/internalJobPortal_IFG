...
// Tambahkan styling kustom untuk date picker
...
export default function LowonganContent() {
  ...
  // Remove the candidateNote state
  ...
  const handleSaveJob = async () => {
    if (!user || (user.role !== "superadmin" && user.role !== "hrd")) {
      toast({
        title: "Akses ditolak",
        description: "Anda tidak memiliki izin untuk menyimpan lowongan.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (
      isEditMode &&
      user.role === "hrd" &&
      user.companyId !== newJob.companyId
    ) {
      toast({
        title: "Akses ditolak",
        description: "Anda tidak memiliki izin untuk mengedit lowongan ini.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      setNewJob({
        ...
        location: "",
        description: "",
        companyId: user.role === "superadmin" ? 1 : user.companyId,
        postingDate: new Date().toISOString(),
        expirationDate: new Date(),
        jobType: "Full-time",
        jobQualification: "",
  ...
  const handleDeleteJob = async (jobId: number) => {
    if (!user || (user.role !== "superadmin" && user.role !== "hrd")) {
      toast({
        title: "Akses ditolak",
        description: "Anda tidak memiliki izin untuk menghapus lowongan ini.",
        variant: "destructive",
      });
      return;
    }
    ...
    if (
      !jobToDelete ||
      (user.role === "hrd" && user.companyId !== jobToDelete.companyId)
    ) {
      toast({
        title: "Akses ditolak",
        description: "Anda tidak memiliki izin untuk menghapus lowongan ini.",
  ...
  const handleAddCandidate = async () => {
    ...
    try {
      ...
      // Refresh the applications data and employee data
      fetchData();
    } catch (error) {
  ...
  return (
    <div className="p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        {user && (user.role === "superadmin" || user.role === "hrd") && (
                              ...
                              <div className="space-y-4">
                                <div className="grid grid-cols-5 items-center gap-4">
                                  ...
                                  <div className="col-span-4">
                                    <Select
                                      value={selectedCompanyForCandidate}
                                      onValueChange={
                                        setSelectedCompanyForCandidate
                                      }
                                    >
